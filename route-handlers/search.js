...
export default async function search(req) {
    try {
        ...
        if (!payload || !payload.q)
            throw 'Payload empty or missing q';
        ...
        if (typeof query !== 'string')
            throw 'Payload missing q or not a string';
        if (isNaN(limit))
            throw 'Payload limit not a number';
        if (isNaN(offset))
            throw 'Payload offset not a number';
        ...
        console.error(e);
        return new Response(`Search failed: ${e.message}`, { status: 500 });
    }
}
