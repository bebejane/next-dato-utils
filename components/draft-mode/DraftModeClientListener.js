...
export class DraftModeClientListener extends EventEmitter {
    ...
    updates = 0;
    actions;
    constructor(url, paths, tags, actions) {
        super();
        this.url = url;
        this.paths = paths;
        this.tags = tags;
    ...
    _handleUpdate(event) {
        ...
        if (this.tags?.length === 0 && this.paths?.length === 0)
            return;
        console.log('DraftModeClient: revalidate', 'paths', this.paths, 'tags', this.tags);
        startTransition(() => {
            if (this.tags?.length)
                this.actions.revalidateTag(this.tags);
            if (this.paths?.length)
                this.actions.revalidatePath(this.paths, 'page');
        });
    }
...
//# sourceMappingURL=DraftModeClientListener.js.map