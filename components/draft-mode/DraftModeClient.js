...
export default function DraftMode({ enabled, url: _url, tag, path, actions, position, secret, }) {
    ...
    return (_jsx(_Fragment, { children: _jsxs(Modal, { children: [_jsxs("div", { className: s.draft, style: {
                    ...
                    }, children: [isDev && (_jsx("a", { href: `/api/draft?secret=${secret ?? ''}&slug=${path}${!enabled ? '' : '&exit=1'}`, className: s.link, children: _jsx("button", { "aria-checked": enabled, className: s.button, children: enabled ? 'Draft' : 'Draft' }) })), loading && _jsx("div", { className: s.loader })] }), enabled && (_jsx(ContentLink, { currentPath: pathname, onNavigateTo: () => {
