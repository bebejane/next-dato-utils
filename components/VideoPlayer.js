...
export default function VideoPlayer({ data, className, videoClassName, loop = true, muted = true, autoPlay = true, contols = false, }) {
    if (!data?.video)
        return null;
    ...
    return (_jsxs("div", { className: className, children: [_jsx("video", { ref: videoRef, src: quality ? data.video[`mp4${quality}`] : data.video.streamingUrl, className: videoClassName, muted: muted, loop: loop, autoPlay: autoPlay, playsInline: true, disablePictureInPicture: true, poster: showPoster ? `${data.video?.thumbnailUrl}?time=0` : undefined }), contols && _jsx("button", { onClick: handleClick, children: playing ? _jsx(PauseButton, {}) : _jsx(PlayButton, {}) })] }));
}
