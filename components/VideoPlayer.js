...
export default function VideoPlayer({ data, className, videoClassName, loop = true, muted = true, autoPlay = true, controls = false, }) {
    ...
    return (_jsxs("div", { className: className, children: [_jsx("video", { ref: videoRef, src: quality ? data.video[`mp4${quality}`] : data.video.streamingUrl, className: videoClassName, muted: muted, loop: loop, autoPlay: autoPlay, playsInline: true, disablePictureInPicture: true, poster: showPoster ? `${data.video?.thumbnailUrl}?time=0` : undefined, style: controls ? { position: 'relative' } : undefined }), controls && (_jsx("button", { style: buttonStyle, onClick: handleClick, children: playing ? _jsx(PauseButton, {}) : _jsx(PlayButton, {}) }))] }));
}
