...
export default async function apiQuery<T, V = void>(
	query: DocumentNode,
	options?: ApiQueryOptions<V>
): Promise<T & { draftUrl: string | null }> {
	...
	const res = opt.includeDrafts
		? await dedupedFetch({ ...dedupeOptions, tags, url: 'https://graphql-listen.datocms.com/preview' })
		: {};
...
const paginatedQuery = async <T, V = void>(
	query: DocumentNode,
