...
export const findConfig = (): string => {
	...
	const searchPaths =
		process.env.NODE_ENV === 'production' ? [configPath, outPath, srcPath, rootPath] : [configPath, srcPath, rootPath];
	...
	throw new Error(
		'Error: cannot find DatoCms config. Please create a configuration file located at the root of your current working directory called "datocms.config.js" or "datocms.config.ts".'
	);
};
