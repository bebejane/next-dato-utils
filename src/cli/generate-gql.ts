...
// Define interfaces for link validators to satisfy TypeScript
interface ItemItemTypeValidator {
	item_types: string[];
}

// Define interface for structured text block validator
interface StructuredTextBlockValidator {
	item_types: string[];
}
interface ItemsItemTypeValidator {
	item_types: string[];
}
...
export default async function generateGqlFiles() {
	try {
		...
		const modelMap = new Map<string, { apiKey: string; pascalName: string }>();
			...
			for (const field of fields) {
				...
				switch (fieldType) {
					case 'structured_text': {
						...
						if (blockValidators && (blockValidators as StructuredTextBlockValidator).item_types && (blockValidators as StructuredTextBlockValidator).item_types.length > 0) {
							const blockModelIds = (blockValidators as StructuredTextBlockValidator).item_types;
							blocksQuery += ` {
`;
									...
									for (const blockField of blockFields) {
										...
										switch (blockFieldType) {
											...
											case 'links': {
												...
												if (linkValidator && (linkValidator as ItemItemTypeValidator | ItemsItemTypeValidator).item_types && (linkValidator as ItemItemTypeValidator | ItemsItemTypeValidator).item_types.length > 0) {
													const linkedModelIds = (linkValidator as ItemItemTypeValidator | ItemsItemTypeValidator).item_types;
													let fragmentSpread = '';
